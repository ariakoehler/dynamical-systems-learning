...
\item
  $f^*(x) = f_0(x) + m^*(x)$: True full model, including 

\item
  $f(x, \eta) = f_0(x) + m(x, \eta)$: Predicted model 
...
If we look for an approximation $f$ that approximates $f^*$ optimally under some loss $L(f^*, f)$ (further details on the loss later), we can identify a few main sources of loss that we expect to contribute

%% If we consider a general loss function for evaluating a model, say $L(f^*, f)$, where $f$ is the true model, 
...
The first problem we are interested in is estimating $\eta$. That is, given $f(x;\eta)$ and a set of observations $y_1, \ldots, y_N$, can we find $\eta$ to within a reasonable error margin?
